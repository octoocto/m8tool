...
impl AsTaskParams for ShrinkTaskParams {
...
impl From<TaskParams> for ShrinkTaskParams {
    fn from(params: TaskParams) -> Self {
        Self {
            ...
            whitelisted_dirs: vec!["samples".to_string(), "packs".to_string()],
...
impl ShrinkTask {
    ...
    pub fn from_params(params: ShrinkTaskParams) -> Result<Self, Error> {
...
impl Task for ShrinkTask {
    ...
    fn start(&mut self) -> Result<(), Error> {
        ...
        self.paths = collect_paths_to_shrink(&source_path, &self.params);
        ...
        self.handle = Some(std::thread::spawn(move || {
            ...
            for path in paths {
                ...
                let minimized_path = shrink_path(&source_path, &relative_path.to_path_buf())?;
                let backup_file_path = task_backup_path.join(&relative_path);
                ...
                {
                    message_tx.log(style(path.display()));
                    message_tx.log(style(new_file_path.display()).green());
            ...
            if is_dry_run {
                message_tx.log(style("note: dry run, nothing was actually done").yellow());
            }

            Ok(TaskResult::new(processed_paths, renamed_paths))
        }));
...
fn collect_paths_to_shrink(
    source_path: &PathBuf,
    shrink_params: &ShrinkTaskParams,
) -> Vec<PathBuf> {
    // only collect paths to .wav files that are in the whitelisted directories
    crate::collect_paths(source_path)
        ...
        .filter(|path| {
            if let Ok(path) = path.strip_prefix(source_path) {
                is_path_in_dir_whitelist(path, &shrink_params.whitelisted_dirs).unwrap_or(false)
            } else {
                false
            }
        })
...
/// "samples/sample.wav")
fn shrink_path(input_path: &PathBuf, path: &PathBuf) -> Result<PathBuf, Error> {
    ...
    let remove_common_prefixes = true;
