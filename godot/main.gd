...
enum SourcePathMode { DRIVE, DIRECTORY }
...
func _ready() -> void:
	...
	self.check_source_drive_mode.button_pressed = self.config_file.get_value("settings", "source_path_mode", SourcePathMode.DRIVE)
	...
	for option: String in ["backup", "convert", "shrink", "clean"]:
...
func _update_progress_bar(percent: float, current_file: String) -> void:
	if percent >= 0.0:
		...
		self.progress_status_left.text = "%.2f%%" % (percent * 100.0)
...
func start_tasks(dry_run: bool) -> void:
	...
	output_label.append_text("[color=green]source path:[/color] %s\n" % self.source_path)
	output_label.append_text("[color=green]backup path:[/color] %s\n" % self.backup_path)
	...
	if self.tree_options.is_checked("convert"):
		self.tasks.add_convert_task(
			self.tree_options.get_convert_whitelisted_dirs(),
			self.tree_options.get_convert_target_bit_depth(),
			self.tree_options.get_convert_target_sample_rate(),
			self.tree_options.get_convert_from_dual_mono(),
			self.tree_options.get_convert_other_formats(),
		)

	if self.tree_options.is_checked("shrink"):
		self.tasks.add_shrink_task(
			self.tree_options.get_shrink_whitelisted_dirs(),
	...
	output_label.append_text("[color=green]created task list:[/color] %s\n" % " -> ".join(task_names))
