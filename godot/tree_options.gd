...
func _ready() -> void:
	...
	_create_item_check(root, "optimize", "optimize samples", true)
	...
	_create_item_option(
		"optimize",
		"optimize_bit_depth",
		"target bit depth (bits)",
		["keep", "8", "16", "24", "32"],
		2,
	)
	_create_item_option(
		"optimize",
		"optimize_sample_rate",
		"target sample rate (Hz)",
		["keep", "44100", "48000"],
		1,
	)
	_create_item_check("optimize", "optimize_dual_mono", "convert dual mono samples to mono", true)

	# _create_item_check("convert", "convert_formats", "convert non-wav files to wav", true)
	_create_item_text(
		"shrink",
		"shrink_whitelist",
		"list of subdirectories to scan",
	...
	_create_item_check(
		"shrink",
		"shrink_remove_common_prefixes",
		"remove common prefixes in name",
		true,
	)

	self.item_map["backup"].set_tooltip_text(
		0,
		"creates an incremental backup of the drive using rsync",
	)
	self.item_map["optimize"].set_tooltip_text(
		0,
		"converts samples to a target bit depth and sample rate using ffmpeg",
	)
	...
	if M8Tool.which("ffmpeg") == "" or M8Tool.which("ffprobe") == "":
		self.item_map["optimize"].set_checked(0, false)
		self.item_map["optimize"].set_editable(0, false)
		self.item_map["optimize"].set_text(0, "optimize samples (ffmpeg and/or ffprobe not found)")
		var child := self.item_map["optimize"].get_first_child()
		while child != null:
...
func get_optimize_whitelisted_dirs() -> PackedStringArray:
	if "optimize_whitelist" not in self.item_map:
		return PackedStringArray()
	var text := self.item_map["optimize_whitelist"].get_text(1)
	var dirs := text.split(",", false)
	for i in range(dirs.size()):
		dirs[i] = dirs[i].strip_edges()
		dirs[i] = dirs[i].rstrip("/\\")
...
func get_optimize_target_bit_depth() -> int:
	if "optimize_bit_depth" not in self.item_map:
		return 0

	match int(self.item_map["optimize_bit_depth"].get_range(1)):
		1:
			return 8
		2:
			return 16
...
func get_optimize_target_sample_rate() -> int:
	if "optimize_sample_rate" not in self.item_map:
		return 0

	match int(self.item_map["optimize_sample_rate"].get_range(1)):
		1:
			return 44100
		2:
			return 48000
...
func get_optimize_from_dual_mono() -> bool:
	if "optimize_dual_mono" not in self.item_map:
		return false
	return self.item_map["optimize_dual_mono"].is_checked(0)


func get_convert_formats() -> bool:
	if "convert_formats" not in self.item_map:
		return false
	return self.item_map["convert_formats"].is_checked(0)
...
func _create_item_option(
	parent: Variant,
	key: String,
	text: String,
	item_options: PackedStringArray,
	default: int,
) -> TreeItem:
	var item := _create_item(parent, key)
	item.set_cell_mode(1, TreeItem.CELL_MODE_RANGE)
